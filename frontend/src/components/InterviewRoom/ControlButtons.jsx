...
const ControlButtons = ({
  ...
  onMuteToggle,
  onVideoToggle,
  disabled,          // controls mic/mic-switcher only
  onMicChange,
  currentMicId,
  room,
  isAgentSpeaking,
  ...
  const generateBars = () => {
    ...
    for (let i = 0; i < numBars; i++) {
      ...
      bars.push(
        <div
          key={i}
          className={`w-1 rounded-full transition-all duration-100 ${isActive ? 'bg-green-500' : 'bg-gray-600'}`}
  ...
  // ─── disabled controls mic; video is ALWAYS enabled ───────────────────────
  const micDisabled = disabled;
  ...
  return (
    <div className="rounded-lg bg-base-300 p-2 sm:p-3 w-full">
      <div className="flex items-center justify-between gap-2 sm:gap-3 md:gap-4 lg:gap-6">
        ...
        {/* Mic controls — disabled before connecting */}
        <div className="flex items-center gap-1.5 sm:gap-2">
          <div className="relative">
            <button
              onClick={onMuteToggle}
              disabled={micDisabled}
              className={`btn btn-sm h-10 sm:h-11 md:h-12 px-3 sm:px-3.5 md:px-4 rounded-lg transition-all bg-base-100 hover:bg-base-100 text-white ${micDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <div className="relative">
            <button
              ...
              disabled={micDisabled || isAgentSpeaking}
              className={`btn btn-sm h-10 sm:h-11 md:h-12 px-2 sm:px-2.5 md:px-3 rounded-lg bg-base-100 hover:bg-base-200 text-white transition-all ${(micDisabled || isAgentSpeaking) ? 'opacity-50 cursor-not-allowed' : ''}`}
              title={isAgentSpeaking ? "Cannot change mic while agent is speaking" : "Select microphone"}
            >
              <ChevronDown className={`h-3.5 w-3.5 sm:h-4 sm:w-4 transition-transform ${showMicDropdown ? 'rotate-180' : ''}`} />
            </button>

            {showMicDropdown && !micDisabled && !isAgentSpeaking && (
              <div className="absolute bottom-full mb-2 right-0 w-56 sm:w-60 md:w-64 lg:w-72 bg-base-300 rounded-lg shadow-xl z-50 max-h-52 sm:max-h-60 md:max-h-64 overflow-y-auto">
                <div className="py-2">
                  <div className="px-3 sm:px-4 py-2 text-xs text-gray-400 font-semibold uppercase">Select Microphone</div>
                  <div
                    onClick={() => handleMicChange('')}
                    className={`px-3 sm:px-4 py-2 hover:bg-base-100 cursor-pointer transition-colors ${!currentMicId ? 'bg-base-100 text-white' : 'text-gray-300'}`}
                  >
                  ...
                  {mics.map((mic) => (
                    <div
                      ...
                      className={`px-3 sm:px-4 py-2 hover:bg-base-100 cursor-pointer transition-colors ${currentMicId === mic.deviceId ? 'bg-base-100 text-white' : 'text-gray-300'}`}
                    >
        ...
        {/* Video button — ALWAYS enabled regardless of connection */}
        <button
